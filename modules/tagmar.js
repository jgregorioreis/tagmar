...
function boasVindas () {
  if(!game.user.getFlag("tagmar","boasVindas")) {
    let contento = '<img src="/systems/tagmar/templates/sheets/img/logo.png" style="display: block; margin-left: auto; margin-right: auto; border-width: 0;" /><h1 class="fairyDust rola" style="text-align:center;">Tagmar RPG</h1>';
...
function setInf_ataque(target_token, user) {
  if (user == game.user) {
    ...
    chatData.content = "<p><img src='"+ actor.img +"' style='float: left; margin-left: auto; margin-right: auto; width: 40%;border: 0px;' /><img src='systems/tagmar/assets/TAGMAR FOUNDRY.png' style='float: left;margin-top:25px; margin-left: auto; margin-right: auto; width: 20%;border: 0px;'/><img src='"+ target_token.actor.img +"' style='float: left; width: 40%; margin-left: auto; margin-right: auto;border: 0px;' /></p><p class='rola_desc mediaeval' style='display: block;margin-left:auto;margin-right:auto;margin-top:60%;'>"+ "<b>Agressor: </b>" + actor.name + "<br><b>Bônus de Ataque: </b>"+ String(actor.system.inf_ataque.bonus) +"<br><b>Oponente: </b>" + target_token.actor.name  +"<br><b>Def. Oponente: </b>"+ target_def.categoria + String(target_def.valor) +"</p>";
...
Hooks.on("renderCombatTracker",function (combatTracker, html) {
  if (combatTracker.combats.length > 0) {
    if (!combatTracker.options.popOut && game.settings.get('tagmar', 'popOutCombat')) combatTracker.renderPopout();
  } 
  if (!game.user.isGM) return;
